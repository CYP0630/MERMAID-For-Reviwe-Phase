...
class CheckAgent:
    ...
    def __init__(self, meta_model: str, exec_model: str, memory_file: str = None):
        """
        ...
        Args:
            ...
            memory_file: Path to memory storage file (optional)
        """
        self.max_react_cycles = 5

        # Initialize evidence memory
        self.memory = EvidenceMemory(memory_file or "memory/evidence_memory.json")

        if meta_model in ["gpt-4o", "gpt-5-mini", "gpt-5.2", "gpt-5"]:
        ...
        if exec_model in ["gpt-4o", "gpt-5-mini", "gpt-5.2", "gpt-5"]:
    ...
    def _extract_keywords(self, triplets: List[Dict]) -> List[str]:
        """
        Extract keywords from triplets for memory matching.

        Args:
            triplets: List of triplet dictionaries with subject, relation, object

        Returns:
            List of keywords (subjects and objects from triplets)
        """
        keywords = []
    ...
    async def react_loop(self, query: str, background: Dict) -> Tuple[str, List[str]]:
        ...
        # Extract keywords from triplets for memory operations
        keywords = self._extract_keywords(background.get("triplets", []))

        # Retrieve relevant evidence from memory
        relevant_evidence = self.memory.retrieve(keywords, top_k=3)

        # Initialize history with retrieved evidence (if any)
        history = []
        if relevant_evidence:
            logging.info(f"Found {len(relevant_evidence)} relevant evidence entries in memory")
            for ev in relevant_evidence:
                evidence_snippet = ev['evidence'][:2000] + "..." if len(ev['evidence']) > 2000 else ev['evidence']
                history.append(f"[Memory] Previous evidence for '{ev['query']}': {evidence_snippet}")
...
async def main():
    ...
    parser.add_argument("--dataset", default="scifact",
                        choices=["factscore", "factool_qa", "factcheckbench", "bingcheck", "scifact"], help="Dataset")
    parser.add_argument("-m", "--meta_model", type=str, default="gpt-4o",
                        help="Model for decompose agent")
    parser.add_argument("-e", "--exec_model", type=str, default="gpt-5-mini",
                        help="Model for ReAct agent")
    parser.add_argument("--log-file", type=str, default="logs/",
                        help="Path to write detailed logs")
    parser.add_argument("--memory-file", type=str, default="memory/scifact_evidence_memory.json",
                        help="Path to memory storage file for evidence caching")
    args = parser.parse_args()
    
    if args.exec_model == "openai/gpt-oss-20b":
        model_name = "gpt-oss-20b"
    elif args.exec_model == "openai/gpt-oss-120b":
        model_name = "gpt-oss-120b"
    elif args.exec_model == "Qwen/Qwen2.5-72B-Instruct-Turbo":
        model_name = "qwen2.5-72b"
    elif args.exec_model == "Qwen/Qwen2.5-7B-Instruct-Turbo":
        model_name = "qwen2.5-7b"
    elif args.exec_model == "meta-llama/Meta-Llama-3.1-70B-Instruct-Turbo":
        model_name = "meta-llama-70b"
    else:
        model_name = args.exec_model
        
    # Initialize logging
    log_file_path = os.path.join(args.log_file, f"{args.dataset}_{model_name}_with_memory.log")
    ...
    server_path = [
        ...
        "src/server/googlescholar_tool.py",
        "src/server/papersearch/server.py"
    ]

    # Initialize Agent with memory
    agent = CheckAgent(args.meta_model, args.exec_model, memory_file=args.memory_file)
    ...
    output_path = f"results/{args.dataset}_{model_name}_with_memory.json"
