...
class CheckAgent:
    ...
    async def react_loop(self, query: str, background: Dict) -> Tuple[str, List[str]]:
        ...
        for current_step in range(self.max_react_cycles):
            ...
            if tool_calls:
                # Extract thought from content if available (for proper ReAct logging)
                thought = ""
                if content:
                ...
                for call in tool_calls:
                    ...
                    logging.info(f"Action: Calling tool: {t_name} with args: {t_args}")
                    ...
                    logging.info(f"Observation: {observation_truncated[:500]}...")  # Log truncated observation
                continue

            # Parse JSON response for ReAct decision (when LLM doesn't use tool_calls)
            if content:
                # Use robust JSON extraction that handles embedded JSON in text
                parsed = extract_json_from_text(content)
                ...
                if parsed:
                    ...
                    if thought:
                        logging.info(f"Thought: {thought[:500]}...")
                    ...
                    if "action" in parsed:
                        ...
                        if actual_tool in self.sessions:
                            ...
                            history.append(f"Observation: {observation_truncated}")
                            logging.info(f"Observation: {observation_truncated[:500]}...")
                        else:
...
async def main():
    ...
    parser.add_argument("--log-file", type=str, default="results/interactive_results.log",
