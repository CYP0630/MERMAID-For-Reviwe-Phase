...
mcp = FastMCP(
    ...
    instructions="""Google Scholar MCP Server for fact-checking tasks."""
    )
...
class GoogleScholarAPI:
    ...
    def google_scholar_search(self, query: str, num_results: int = 5) -> Dict:
        """
        Search Google Scholar and return relevant articles
...
search_api = GoogleScholarAPI()
@mcp.tool(name="search_google_scholar", description="Retrieve Google Scholar search results for a given query.")
def search_google_scholar(query: str, max_results: int = 10) -> Dict:
    """
    Search from Google Scholar related to a query.
    Args:
        query: The search query string
        max_results: Maximum number of results to return (default: 10)

    Returns:
        Dictionary containing Google Scholar search results with titles, links, snippets, and metadata
    """
    return search_api.google_scholar_search(query, max_results)
