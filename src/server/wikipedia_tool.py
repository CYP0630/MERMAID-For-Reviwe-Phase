...
# result = wikipedia_client.search(query="Robert Koch was a German physician and microbiologist")
# print(result)
mcp = FastMCP(
        name="Wikipedia",
        instructions="""Wikipedia MCP Server for fact-checking tasks."""
    )
...
def summarize_article_for_query(
    title: str,
    query: str,
    max_length: Annotated[int, Field(title="Max Length")] = 500,
) -> Dict[str, Any]:
    """
    Get a summary of a Wikipedia article tailored to a specific query.
...
def summarize_article_section(
    title: str,
    section_title: str,
    max_length: Annotated[int, Field(title="Max Length")] = 300,
) -> Dict[str, Any]:
    """
    Get a summary of a specific section of a Wikipedia article.
